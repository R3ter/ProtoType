...
const Mutation = {
  ...
  refreshToken: (parent, { userId, refreshToken }, { prisma }) =>
    RefreshToken(userId, refreshToken, prisma),
  resendActivationCode: (parent, args, { req }) =>
    ResendActivationCode(req.headers.token),
  activateAccount: (parent, { code }, { req, prisma }) =>
    checkActivationCode(
      code,
      req.headers.token,
      async ({ id, role }) => {
        const { full_name, email, phone_number, Role } =
          await prisma.user.update({ where: { id }, data: { Active: true } });
        const info = await loginToken({
          userid: id,
          role: Role,
          Activate: true,
          email,
